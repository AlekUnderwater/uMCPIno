...
Offset
   ...
   * To test this skatch you need two arduino boards
   * Their default Serial ports are supposed to be connected to the Host (a terminal application)
   * The boards are connected to each other via 3 wires (RX, TX and GND)
   * Make sure that one of your boards has CFG_SELECT_DEFAULT_STATE is set to true, and another one to false
   * If your board(s) has physical Serial1 (e.g. Mega 2560) - uncomment the line "#define HAS_SERIAL1"
   * If your board(s) has only one physical serial port (e.g. Uno) - comment the line "#define HAS_SERIAL1", and use Software serial.
     for this purpose check out the SOFTWARE_SERIAL_TX and RX pins
*/
...
void On_NewByte_From_Host(byte c)
{
...
void DC_Input_Process()
{
  ...
  {
    On_NewByte_From_Host(Serial.read());
  }
